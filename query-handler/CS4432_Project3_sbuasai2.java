...
public class CS4432_Project3_sbuasai2 {
  ...
  private static String getRecord(String data, int recordNum) {
    // Substring from record num to number of bytes + record num, giving one full
    // record
  ...
  // main hashing function, simply takes randomV mod numBuckets to get bucket
  // value/address
  ...
  private static void putInt(int key, int value) {
    ...
    // if the bucket is empty, initialize it
    if (tableAInt.get(bucketValue) == null)
      tableAInt.set(bucketValue, new LinkedList<Integer>());
  ...
  // kind of a hack, but we don't need arguments since there is only one
  // configuration we check for
